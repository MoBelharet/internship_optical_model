...
module pisces_migration_weight_distribution
...
type, extends(type_base_model), public ::type_pisces_migration_weight_distribution
    ...
    !type (type_surface_diagnostic_variable_id) :: id_diag_parmean0
...
subroutine depth_integral_initialize(self, configunit)
    class (type_pisces_migration_weight_distribution), intent(inout), target :: self
...
subroutine depth_integral_do_column(self, _ARGUMENTS_DO_COLUMN_)
   ...
   ! _VERTICAL_LOOP_BEGIN_
       ...
       !write(*,*)'weight_distribution : '
       !write(*,*)'parmean0 = ', parmean0
       !write(*,*)'parmean = ', parmean
       !write(*,*)'local = ', local
       integral = integral + local*thickness
       totaldepth = totaldepth + thickness
       !write(*,*)'integral_after = ', integral
       ...
       if (parmean0 < 1E-4_rk) then
            if (totaldepth <= 400.0_rk) then
                local_random = (minimum_value + (1.0_rk - minimum_value) * local_random) * thickness
            else
                local_random = minimum_value * thickness
            end if 
       else
            local_random = (minimum_value + (1.0_rk - minimum_value) * local_random * present) * thickness
       end if
       !write(*,*)local_random
    ...
    !_SET_SURFACE_DIAGNOSTIC_(self%id_diag_parmean0, parmean0)
