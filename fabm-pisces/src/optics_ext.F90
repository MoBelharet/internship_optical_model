...
module pisces_optics_ext
   ...
   type, extends(type_base_model), public :: type_pisces_optics_ext
      type (type_dependency_id)                  :: id_chltot, id_e3t_n
      type (type_dependency_id)                  :: id_r, id_g, id_b, id_r_mean, id_g_mean, id_b_mean
...
contains
   ...
   subroutine initialize(self, configunit)
      class (type_pisces_optics_ext), intent(inout), target :: self
      ...
      call self%register_dependency(self%id_r, 'r', 'W m-2', 'instantaneous PAR in red band, 600-700 nm [in ice-free water]')
      call self%register_dependency(self%id_g, 'g', 'W m-2', 'instantaneous PAR in green band, 500-600 nm [in ice-free water]')
      call self%register_dependency(self%id_b, 'b', 'W m-2', 'instantaneous PAR in blue band, 400-500 nm [in ice-free water]')
      call self%register_dependency(self%id_r_mean, temporal_mean(self%id_r, period=86400._rk,resolution=3600._rk, missing_value=0.0_rk))
      call self%register_dependency(self%id_g_mean, temporal_mean(self%id_g, period=86400._rk,resolution=3600._rk, missing_value=0.0_rk))
      call self%register_dependency(self%id_b_mean, temporal_mean(self%id_b, period=86400._rk,resolution=3600._rk, missing_value=0.0_rk))
   ...
   subroutine do_column(self, _ARGUMENTS_DO_COLUMN_)
      class (type_pisces_optics_ext), intent(in) :: self
      ...
      logical :: first
      real(rk) :: r_rad, g_rad, b_rad, r_mean_rad, g_mean_rad, b_mean_rad
      ...
      _DOWNWARD_LOOP_BEGIN_
         ...
         _GET_(self%id_r, r_rad)
         _GET_(self%id_g, g_rad)
         _GET_(self%id_b, b_rad)
         _GET_(self%id_r_mean, r_mean_rad)
         _GET_(self%id_g_mean, g_mean_rad)
         _GET_(self%id_b_mean, b_mean_rad)
